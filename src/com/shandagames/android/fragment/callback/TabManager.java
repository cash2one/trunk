...
public class TabManager implements TabHost.OnTabChangeListener {
	...
	/**
	 * A dummy {@link TabContentFactory} that creates an empty view to satisfy
	 * the {@link TabHost} API.
	 * 
	 * @author Jimmy Shih
	 */
	...
	public void addTab(TabSpec tabSpec, Class<?> clss, Bundle bundle) {
		 ...
		 */
		tabInfo.fragment = fragmentActivity.getSupportFragmentManager()
			...
			if (newTabInfo != null) {
				if (newTabInfo.fragment == null) {
					newTabInfo.fragment = Fragment.instantiate(
							fragmentActivity, newTabInfo.clss.getName(),
							newTabInfo.bundle);
					fragmentTransaction.add(containerId, newTabInfo.fragment,
							newTabInfo.tag);
				} else {
