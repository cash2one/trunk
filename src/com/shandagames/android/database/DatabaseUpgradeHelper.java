...
import com.shandagames.android.support.ArraySupport;
...
public final class DatabaseUpgradeHelper {
			...
			while (!cur.isAfterLast()) {
				...
				for (int i = 0; i < length; i++) {
					...
					final String col_alias = column_alias != null && column_alias.containsKey(new_col)
							&& ArraySupport.contains(old_cols, column_alias.get(new_col)) ? column_alias.get(new_col)
							: new_col;
					...
					if (ArraySupport.contains(old_cols, col_alias)) {
	...
	private static boolean shouldUpgrade(final String[] old_cols, final String[] old_types, final String[] new_cols,
		...
		if (!ArraySupport.contentMatch(old_cols, new_cols)) return true;
