...
public class ImageCache {
    private static final String TAG = "ImageCache.class";
    ...
    private void init(ImageCacheParams cacheParams) {
        ...
        if (mCacheParams.memoryCacheEnabled) {
            if (DEBUG) Log.d(TAG, "Memory cache created (size = " + mCacheParams.memCacheSize + ")");
    ...
    public Bitmap getBitmapFromMemCache(String data) {
        if (mMemoryCache != null) {
            ...
            if (memBitmap != null) {
            	if (DEBUG) Log.d(TAG, "Memory cache hit");
                return memBitmap;
            }
        }
        return null;
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null) {
                ...
                try {
                    ...
                    if (snapshot != null) {
                    	if (DEBUG) Log.d(TAG, "Disk cache hit");
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null && !mDiskLruCache.isClosed()) {
                try {
                    ...
                    if (DEBUG) Log.d(TAG, "Disk cache cleared");
                } catch (IOException e) {
    ...
    public void flush() {
        synchronized (mDiskCacheLock) {
            if (mDiskLruCache != null) {
                try {
                    mDiskLruCache.flush();
                    if (DEBUG) Log.d(TAG, "Disk cache flushed");
                } catch (IOException e) {
                	Log.e(TAG, "flush - " + e);
                }
            }
            ...
            if (mDiskLruCache != null) {
                try {
                    if (!mDiskLruCache.isClosed()) {
                        mDiskLruCache.close();
                        mDiskLruCache = null;
                        if (DEBUG) Log.d(TAG, "Disk cache closed");
                    }
                } catch (IOException e) {
                	Log.e(TAG, "close - " + e);
                }
